...
try {
    ...
    // Prima controlla la struttura della tabella admins
    ...
    if (!$adminExists) {
        // Inserisco nella tabella admins con i campi standard
        $stmt = $pdo->prepare("INSERT INTO admins (name, email, password, created_at, updated_at) VALUES (?, ?, ?, ?, ?)");
        $stmt->execute(['Admin', $adminEmail, $adminPassword, $now, $now]);
