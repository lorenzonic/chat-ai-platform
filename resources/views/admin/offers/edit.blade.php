...
<div class="max-w-4xl mx-auto p-6">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-gray-900">Modifica Offerta: {{ $offer->title }}</h1>
    ...
    <form method="POST" action="{{ route('admin.offers.update', $offer) }}" class="bg-white shadow-sm rounded-lg p-6">
        @csrf
        @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="description"
                              ...
                              rows="3"
                              class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-emerald-500"
                              placeholder="Descrizione dettagliata dell'offerta...">{{ old('description', $offer->description) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <select id="grower_id"
                        ...
                        @foreach($growers as $grower)
                            <option value="{{ $grower->id }}" {{ old('grower_id', $offer->grower_id) == $grower->id ? 'selected' : '' }}>
                                {{ $grower->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="md:col-span-2">
                    <label class="flex items-center">
                        ...
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $offer->is_active) ? 'checked' : '' }} class="mr-3">
                        <span class="text-sm font-medium text-gray-700">Offerta attiva</span>
                    </label>
                </div>
            </div>
        </div>
        ...
        <div class="mb-8">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <div class="space-y-3">
                        <label class="flex items-center">
                            <input type="radio" name="type" value="percentage" class="mr-3" {{ old('type', $offer->type) == 'percentage' ? 'checked' : '' }} required>
                            <span class="text-sm">Percentuale di sconto</span>
                        </label>
                        <label class="flex items-center">
                            <input type="radio" name="type" value="fixed_amount" class="mr-3" {{ old('type', $offer->type) == 'fixed_amount' ? 'checked' : '' }} required>
                            <span class="text-sm">Importo fisso di sconto</span>
                        </label>
                        <label class="flex items-center">
                            <input type="radio" name="type" value="buy_x_get_y" class="mr-3" {{ old('type', $offer->type) == 'buy_x_get_y' ? 'checked' : '' }} required>
        ...
        <div class="mb-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Utilizzi Attuali</label>
                    <div class="w-full px-3 py-2 bg-gray-50 border border-gray-300 rounded-md text-gray-500">
                        {{ $offer->usage_count }} utilizzi
                    </div>
                </div>
            </div>
        </div>
                ...
                <div>
                    ...
                    <select name="applicable_products[]" multiple class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-emerald-500" size="5">
                        @foreach($products as $product)
                            <option value="{{ $product->id }}" {{ in_array($product->id, old('applicable_products', $offer->applicable_products ?? [])) ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select name="applicable_categories[]" multiple class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-emerald-500" size="4">
                        @foreach($categories as $category)
                            <option value="{{ $category }}" {{ in_array($category, old('applicable_categories', $offer->applicable_categories ?? [])) ? 'selected' : '' }}>
                                {{ $category }}
                            </option>
                        @endforeach
                    </select>
