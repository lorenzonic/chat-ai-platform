...
class GeographicTestDataSeeder extends Seeder
{
    public function run()
    {
        ...
        if (!$store) {
            $this->command->error('Store not found. Please create a test store first.');
            return;
        }
        ...
        foreach ($testData as $location) {
            $this->command->info("Adding data for {$location['city']}...");
        ...
        $this->command->info('Test geographic data added successfully!');
        $this->command->info('Total interactions: ' . Interaction::where('store_id', $store->id)->count());
        $this->command->info('Total leads: ' . Lead::where('store_id', $store->id)->count());
        $this->command->info('Geographic interactions: ' . Interaction::where('store_id', $store->id)->whereNotNull('latitude')->count());
        $this->command->info('Geographic leads: ' . Lead::where('store_id', $store->id)->whereNotNull('latitude')->count());
    }
}
